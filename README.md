...
Make sure to update the nameservers for the newly created subdomain. Must reference the subdomain nameservers from a NS record in the top level domain.
...
```bash
noties$ sam logs -n HelloEfsFunction --stack-name noties --tail
```
...
```bash
noties$ pip install -r tests/requirements.txt --user
# unit test
noties$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
noties$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name noties
```
